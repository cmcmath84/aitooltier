...
export const codex: ToolReview = {
  slug: "codex",
  name: "Codex (OpenAI)",
  tagline: "**Agents API in public beta (2026-09-10)**: the Codex harness -- context compaction, tool search, programmatic tool calling, subagents -- is now a hosted OpenAI API with no additional fee beyond tokens and tools, plus OpenAI-hosted sandboxes and partner environments (Cloudflare, Vercel, Modal, E2B, Daytona, Runloop, Blaxel, DigitalOcean, Oracle). OpenAI's coding agent -- parallel tasks, PRs, and as of 2026-07-09 merged into the unified ChatGPT desktop app (Chat + Work + Codex, every plan incl. Free) with GPT-5.6 across the lineup. **GPT-6 Astra arrived 2026-09-03 with a rewritten Codex harness -- OpenAI claims 1.9x faster task completion vs GPT-5.6 Sol on Mind2Web**, though that figure combines the harness change with the model. GPT-5.4 and 5.4-mini left Codex on 2026-08-31",
  ...
  knownIssues: [
    {
      description: "THE AGENTS API -- CODEX'S HARNESS BECOMES A HOSTED PRODUCT, IN PUBLIC BETA, WITH NO SURCHARGE (2026-09-10, vendor-primary): OpenAI introduced the **Agents API** in public beta, 'bringing that same harness and infrastructure that powers Codex to developers through a simple, flexible API.' **What you get in one call** (`client.beta.agents.sessions.create`): a model (the example is `gpt-6-astra`), tools (MCP servers, custom functions, built-ins like web search), a `multi_agent` block with `max_concurrent_subagents`, secrets via `vault_ids`, and an `environment` -- either **`openai_hosted`** (a new OpenAI-managed sandbox on 'the same sandboxing infrastructure that powers Codex and ChatGPT', configurable with files, packages, skills and plugins) or your own infrastructure or a partner sandbox. **Named sandbox partners:** Blaxel, Cloudflare, Daytona, DigitalOcean, E2B, Modal, Oracle, Runloop, Vercel. **Harness features OpenAI calls out as maintained for you:** automatic **context compaction** as a session nears its window, **tool search** (loads tool definitions on demand to save tokens and preserve cache), **programmatic tool calling** (parallel calls, chaining and in-code filtering so only relevant results re-enter context), and **subagents** with their own context. OpenAI says the harness is 'versioned' and improved with each model launch, so you inherit upgrades without rewriting scaffolding; it is 'powered by the open-source Codex harness', so the coordinating logic is inspectable. **PRICING: 'There are no additional fees for using the Agents API -- you simply pay for the tokens and tools your agents use.'** That is the notable commercial decision: OpenAI is not charging for orchestration or the hosted sandbox line item at beta, which undercuts the agent-infrastructure vendors it is simultaneously partnering with. **CUSTOMER CLAIMS (vendor-selected, unverified):** Ciridae eval score 0.71 -> 0.85 and 4x latency reduction on subagent flows; SafetyKit 60% lower cost per case; Hypha 86% fewer failed agent responses; Nash running 'thousands of long-running AI agents' on it. **HOW TO READ IT ON THIS PAGE:** Codex the product (CLI, IDE, cloud, app) is unchanged; what changed is that its runtime is now a platform primitive, which is the clearest statement yet of where OpenAI thinks the moat is -- the harness and infrastructure rather than the model alone. Beta means the API surface can change before GA; OpenAI says it will 'iterate quickly' on feedback.",
      source: "OpenAI (openai.com/index/introducing-the-agents-api/, publicationDateText 'September 10, 2026'; no 'Originally published on' block) -- fetched 2026-09-14 via curl with browser UA",
      date: "2026-09-10",
    },
    {
      description: "THE CODEX HARNESS WAS REWRITTEN ALONGSIDE GPT-6 ASTRA -- 1.9x FASTER TASK COMPLETION, AND THE SPEED GAIN IS NOT ALL FROM THE MODEL (2026-09-03, vendor-primary): OpenAI shipped **GPT-6 Astra** and states in the launch post: '**Alongside Astra, we are also updating the Codex harness to significantly improve the speed of computer use. Combined with Astra's efficiency, this translates to a 1.9x faster task completion compared to the current GPT-5.6 Sol experience, on the Mind2Web benchmark.**' **Read the attribution carefully, because it is doing work: the 1.9x is the combination of a harness change and a model change, not a model benchmark.** OpenAI separately reports that on OSWorld 2.0 latency simulations Astra scores **72.6% at roughly 40 minutes per task** versus Sol's **65.7% at roughly 75 minutes** -- about **47% less time** -- so the model is genuinely faster on computer use, but the Codex-specific figure bundles the harness update in. **WHAT THIS MEANS IF YOU RUN CODEX:** the harness improvement lands for everyone on the Codex surface, and Astra reaches Plus, Pro, Business and Enterprise within existing subscription allowances as the staged rollout completes. Astra is API-addressable as `gpt-6-astra` at **$10/$50 per 1M short-context** ($20/$75 long-context, Batch $5/$25, Fast mode 2x) -- **2.5x the price of GPT-5.6 Sol at $4/$20**, which is the trade to weigh for high-volume agent runs where Sol was already adequate. **CONTEXT WITH THIS WINDOW'S OTHER CODEX CHANGE:** GPT-5.4 and GPT-5.4-mini left the Codex surface on 2026-08-31 as scheduled, so within four days the Codex model list lost its two oldest entries and gained OpenAI's most expensive one.",
      source: "OpenAI (openai.com/index/gpt-6-astra/, publicationDateText 'September 3, 2026') + platform.openai.com/docs/pricing.md -- fetched 2026-09-05 via curl with browser UA",
      date: "2026-09-03",
    },
    {
  ...
  dataSources: [
    { name: "OpenAI: Introducing the Agents API -- public beta, Codex harness as a hosted API, OpenAI-hosted sandbox, partner environments, no additional fees (2026-09-10)", url: "https://openai.com/index/introducing-the-agents-api/", dateAccessed: "2026-09-14" },
    { name: "OpenAI: GPT-6 Astra launch -- Codex harness updated, 1.9x faster task completion on Mind2Web (2026-09-03)", url: "https://openai.com/index/gpt-6-astra/", dateAccessed: "2026-09-05" },
    { name: "OpenAI API pricing doc: gpt-6-astra $10/$50 short context vs gpt-5.6-sol $4/$20 (verified 2026-09-05)", url: "https://platform.openai.com/docs/pricing", dateAccessed: "2026-09-05" },
